...
def schema_to_index(schema, index_names=None, prefixed=True):
    """Get index/doc_type given a schema URL.

    :param schema: The schema name
    :param index_names: A list of index name.
    :param prefixed: A boolean which indicates whether the index name should
    ...
    if index_names is None:
        index = build_index_name(parts, suffix='')
        if not prefixed:
            index = index.replace(current_app.config['SEARCH_INDEX_PREFIX'], '')
        return index, doc_type

    for start in range(len(parts)):
        index_name = build_index_from_parts(*parts[start:])
        if index_name in index_names:
            index = build_index_name(index_name, suffix='')
            if not prefixed:
                index = index.replace(current_app.config['SEARCH_INDEX_PREFIX'], '')
            return index, doc_type
